...
pub fn get_port_with_override(override_port: Option<u16>) -> u16 {
    override_port
        .or_else(|| std::env::var("SPYWEB_PORT").ok().and_then(|p| p.parse().ok()))
        .unwrap_or(7979)
}
...
pub fn config_check() -> anyhow::Result<()> {
    ...
    match loader::load_all_jobs("jobs.toml", "jobs", db) {
        Ok(jobs) => {
            println!("Found {} job(s):", crate::color::c_info(&jobs.list.len().to_string()));
            for job in &jobs.list {
                ...
                println!("  [{}] {} ({})", status, crate::color::c_job(&job.config.name), hook_status);
            }
